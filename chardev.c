...
#define BUF_LEN 80		/* Max len of msg from the device */
...
static int device_open(struct inode *inode, struct file *file)
{
	...
	if ( Device_Open )
	{
		return -EBUSY; // asm-generic/errno-base.h
	}
                         ...
                         size_t len, loff_t *ppos)
{
	...
	{
        /*
         * The buffer is in the useer data segment, not the kernel segment
         * so "*" assignment won't work. We have to use put_user which copies data from
